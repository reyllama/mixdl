...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if which == 0:
            ...
            if args.augment:
                # print("Augmentation on the run~~~~~~~")
                fake_img = DiffAugment(fake_img, policy='color,translation,cutout')
        ...
        if args.augment:
            real_img_aug = DiffAugment(real_img, policy='color,translation,cutout')
        ...
        if which > 0:
            ...
            if args.augment:
                ...
                # fake_img, _ = augment(fake_img, ada_aug_p)
                interp_img = DiffAugment(interp_img, policy='color,translation,cutout')
                fake_img = DiffAugment(fake_img, policy='color,translation,cutout')
        ...
        if get_rank() == 0:
            ...
            # if i>30000 and i % 50 == 0:
            #     generate(args, g_ema, device, mean_latent, i)

            # if i % args.save_every != 0 and i % 500 == 0:
            #     torch.save(
            #         {
            #             "g_ema": g_ema.state_dict()
            #         },
            #         args.root + f"/checkpoint/{str(i).zfill(6)}_small.pt",
            #     )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="kl weight for discriminator feature alignment"
    )
