...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024):
        ...
        for k, v in nfc_multi.items():
            nfc[k] = int(v*ngf) # (4:1024, 8:512, 16:256, 32:128, 64:128, 128:64, 256:32, 512:16, 1024:8)
        ...
        self.to_big = conv2d(nfc[im_size], nc, 3, 1, 1, bias=False)  # to_rgb layer
    ...
    def forward(self, input, return_feats=False):
        ...
        if return_feats:
            feat_list = [feat_4, feat_8, feat_16, feat_32]

        if self.im_size == 256:
            if return_feats:
                return [self.to_big(feat_256), self.to_128(feat_128)], feat_list
            else:
...
class Discriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512):
        ...
        self.final_linear = linear(8*8*512, 512)
    ...
    def forward(self, imgs, label, part=None):
        ...
        feat_2 = self.down_from_big(imgs[0])        # 256, 256, 16
        feat_4 = self.down_4(feat_2) # 128,128,32
        feat_8 = self.down_8(feat_4) # 64,64,64
        
        feat_16 = self.down_16(feat_8) # 32, 32, 128
        feat_16 = self.se_2_16(feat_2, feat_16)

        feat_32 = self.down_32(feat_16) # 16, 16, 256
        feat_32 = self.se_4_32(feat_4, feat_32)
        
        feat_last = self.down_64(feat_32) # 8, 8, 512
        ...
        if label=="interp":
            batch = feat_last.shape[0]
            lin_feat = feat_last.view(batch, -1)
            lin_feat = self.final_linear(lin_feat) # B, 512
            n_sample = batch // 2
            sims = torch.zeros(n_sample, n_sample).cuda()
            for idx1 in range(n_sample):
                for idx2 in range(n_sample):
                    sims[idx1, idx2] = self.sim(
                        torch.unsqueeze(lin_feat[idx2], 0), torch.unsqueeze(lin_feat[idx1 + n_sample], 0)
                    )
            sims = self.sfm(sims)

            return torch.cat([rf_0, rf_1]), sims
